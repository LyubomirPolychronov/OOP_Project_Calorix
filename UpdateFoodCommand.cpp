...
void UpdateFoodCommand::execute(const std::vector<std::string>& args)
{
	...
	if (!admin)
	{
		throw std::invalid_argument("Only admins can update the calories from the food Data Base");
	}
	if (args.size() < 2)
	{
		throw std::invalid_argument("Usage: update-food <food_name> <new_calories>");
	}
