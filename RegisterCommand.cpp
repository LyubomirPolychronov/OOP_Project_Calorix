...
void RegisterCommand::execute(const std::vector<std::string>& args)
{
	if (args.size() < 7)
	{
		throw InvalidCommand("Usage: register <username> <password> <age> <weight> <height> <gender> <acticity_level>");
	}
	...
	for (const auto& user : Calorix::getInstance().getUserDB()) {
		...
		{
			throw InvalidCommand("Username already exists. Please choose another one");
		}
	}
